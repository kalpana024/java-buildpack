...
Function printData(dataobjItem)
  ...
  if NOT (dataobjItem.Name = "") then
    ...
    IF (processIdCmdArgsMap.Exists(processid)) then
         ...
         End if
    End if 
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & dataobjItem.Channels & " " & dataobjItem.ContextBoundClassesLoaded & " " & dataobjItem.ContextBoundObjectsAllocPersec & " " & dataobjItem.ContextProxies & " " & dataobjItem.Contexts  & " " & dataobjItem.RemoteCallsPersec & " " & dataobjItem.TotalRemoteCalls & vbCrlf
  End If
End Function
