...
Rem this object holds the process class object to get process id and command line argument values
...
Rem Generating map of the Process Id and Process's Command Line Argument value for searcPattern given in monitor syntax
Rem like cmdlineArgument for Prosess [C:\Program Files (x86)\Mozilla Firefox\plugin-container.exe" --channel="6848.7.1482661345\1125907410" -greomni "C:\Program Files (x86)\Mozilla Firefox\omni.ja" -appomni "C:\Program Files (x86)\Mozilla Firefox\browser\omni.ja" -appdir "C:\Program Files (x86)\Mozilla Firefox\browser"  6848 "\\.\pipe\gecko-crash-server-pipe.6848"]
Rem Search pattern given  =  -greomni
Rem Map will hold processId as key and "C:\Program Files (x86)\Mozilla Firefox\omni.ja" as value of search pattern
...
Function printData(objItem)
  ...
  if NOT (objItem.Name = "") then
    ...
    IF (processIdCmdArgsMap.Exists(processID)) then
         ...
         End if
    End if 
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & objItem.ElapsedTime & " " & objItem.HandleCount &" " & objItem.IODataBytesPersec & " " & objItem.IODataOperationsPersec & " " & objItem.IOOtherBytesPersec & " " & objItem.IOOtherOperationsPersec & " " & objItem.IOReadBytesPersec & " " & objItem.IOReadOperationsPersec & " " & objItem.IOWriteBytesPersec & " " & objItem.IOWriteOperationsPersec & " " & objItem.PageFaultsPersec & " " & Round((objItem.PageFileBytes/ByteToMB),3) & " " & Round((objItem.PageFileBytesPeak/ByteToMB),3) & " " & objItem.PercentPrivilegedTime & " " & objItem.PercentProcessorTime & " " & objItem.PercentUserTime & " " & Round((objItem.PoolNonpagedBytes/ByteToMB),3) & " " & Round((objItem.PoolPagedBytes/ByteToMB ),3) & " " & objItem.PriorityBase & " " & Round((objItem.PrivateBytes/ByteToMB),3) & " " & objItem.ThreadCount & " " & Round((objItem.VirtualBytes/ByteToMB),3) & " " & Round((objItem.VirtualBytesPeak/ByteToMB),3) & " " & Round((objItem.WorkingSet/ByteToMB),3) & " " & Round((objItem.WorkingSetPeak/ByteToMB),3) & vbCrlf
  End If
End Function
