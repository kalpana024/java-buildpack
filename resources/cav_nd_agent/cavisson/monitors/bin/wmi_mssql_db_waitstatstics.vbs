...
Set colItems = objRefresher.AddEnum(objWMIService, _
    "Win32_PerfFormattedData_MSSQLSTOREDB_MSSQLSTOREDBWaitStatistics").objectSet
...
Function printData(objItem)
  ...
  if NOT (objItem.Name = "") then
    vectorName = Replace(objItem.Name, " ","_")
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & objItem.Lockwaits & " " & objItem.Logbufferwaits & " " & objItem.Logwritewaits & " " & objItem.Memorygrantqueuewaits & " " & objItem.NonPagelatchwaits & " " & objItem.PageIOlatchwaits & " " & objItem.Pagelatchwaits & " " & objItem.Threadsafememoryobjectswaits & " " & objItem.Transactionownershipwaits & vbCrlf
  End If
End Function
