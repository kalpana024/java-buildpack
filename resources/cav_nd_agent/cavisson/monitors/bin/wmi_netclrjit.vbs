...
Set dataSetItem = objRefresher.AddEnum(objWMIService, _
    "Win32_PerfFormattedData_NETFramework_NETCLRJit").objectSet
...
Function printData(dataobjItem)
  ...
  if NOT (dataobjItem.Name = "") then
    ...
    IF (processIdCmdArgsMap.Exists(processid)) then
         ...
         End if
    End if 
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & Round((dataobjItem.NumberofILBytesJitted/byteToMB),3) & " " & dataobjItem.NumberofMethodsJitted & " " & dataobjItem.PercentTimeinJit & " " & Round(((dataobjItem.ILBytesJittedPersec)/byteToMB) ,3)& " " & dataobjItem.StandardJitFailures & vbCrlf
  End If
End Function
