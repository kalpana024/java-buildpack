...
Rem @Description - This script is used to get stats of Sql server plan cache
Rem @GDF - cm_win_sql_server_plan_cache.gdf
Rem @Mon_Sytntax - #STANDARD_MONITOR QA_Tier>WINTESTENV QA_Tier>WINTESTENV SQLServerPlanCache
Rem---------------------------------------------------------------------------------------------------
...
Set colItems = objRefresher.AddEnum(objWMIService, _
    "Win32_PerfFormattedData_MSSQLSERVER_SQLServerPlanCache").objectSet
...
Function printData(objItem)
  ...
  if NOT (objItem.Name = "") then
    vectorName = Replace(objItem.Name, " ","_")
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & objItem.CacheHitRatio & vbCrlf
  End If
End Function
