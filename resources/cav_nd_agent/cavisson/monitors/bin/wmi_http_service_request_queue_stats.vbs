...
Rem @Purpose : Http Service Request Queue Stats monitor metrics related to requests that are queued by the HTTP Service.
Rem @GDF     :  cm_win_http_service_request_queue_stats.gdf
Rem @Command To Check Counters : wmic path Win32_PerfFormattedData_Counters_HTTPServiceRequestQueues
Rem @Counter : ArrivalRate CacheHitRate CurrentQueueSize RejectedRequests RejectionRate
Rem @Monitor Syntax : STANDARD_MONITOR TierName>Server TierName>ServerName>Instance StandardMonitorName
Rem To get All Queues Data : STANDARD_MONITOR QA_Tier>WindowsMachine QA_Tier>WindowsMachine>QA-INST HttpServiceRequestQueueStats
Rem To get Specific Queues Data : STANDARD_MONITOR QA_Tier>WindowsMachine QA_Tier>WindowsMachine>QA-INST HttpServiceRequestQueueStats /args:vector1,vector2
Rem ---------------------------------------------------------------------------------------------------------------------
...
Set colItems = objRefresher.AddEnum(objWMIService, _
    "Win32_PerfFormattedData_Counters_HTTPServiceRequestQueues").objectSet
...
Function printData(objItem)
  ...
  if NOT (objItem.Name = "") then
    vectorName = Replace(objItem.Name, " ","_")
	vectorName = Replace(vectorName, "?","")
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & objItem.ArrivalRate & " " & objItem.CacheHitRate & " " & objItem.CurrentQueueSize & " " & objItem.RejectedRequests & " " & objItem.RejectionRate & vbCrlf
  End If
End Function
