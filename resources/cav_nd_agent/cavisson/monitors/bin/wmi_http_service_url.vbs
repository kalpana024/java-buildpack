...
Rem @GDF     :  cm_win_HTTP_Service_URL.gdf
Rem @Command To Check Counters : wmic path Win32_PerfFormattedData_Counters_HTTPServiceUrlGroups
Rem @Counter : AllRequests BytesReceivedRate BytesSentRate BytesTransferredRate ConnectionAttempts CurrentConnections GetRequests HeadRequests MaxConnections
Rem @Monitor Syntax : STANDARD_MONITOR TierName>Server TierName>ServerName>Instance StandardMonitorName
Rem To get All Request URL Data : STANDARD_MONITOR QA_Tier>WindowsMachine QA_Tier>WindowsMachine>QA-INST HTTPServiceUrlGroupsStats
Rem To get Specific Request URL  Data : STANDARD_MONITOR QA_Tier>WindowsMachine QA_Tier>WindowsMachine>QA-INST HTTPServiceUrlGroupsStats /args:vector1,vector2
Rem --------------------------------------------------------------------------------------------------------------------
...
Set colItems = objRefresher.AddEnum(objWMIService, _
    "Win32_PerfFormattedData_Counters_HTTPServiceUrlGroups").objectSet
...
' arrayOfvector to holds the vector names provided by the user
...
Function printData(objItem)
  ...
  if NOT (objItem.Name = "") then
    vectorName = Replace(objItem.Name, " ","_")
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & objItem.AllRequests & " " & objItem.BytesReceivedRate & " " & objItem.BytesSentRate & " " & objItem.BytesTransferredRate & " " & objItem.ConnectionAttempts & " " & objItem.CurrentConnections & " " & objItem.GetRequests & " " & objItem.HeadRequests & " " & objItem.MaxConnections & vbCrlf
  End If
End Function
