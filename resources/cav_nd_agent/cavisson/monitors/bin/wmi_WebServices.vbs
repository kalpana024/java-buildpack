...
Rem @GDF - cm_win_WebServices.gdf
Rem @Description - This script is used to get web services stats.
Rem @Monitor Syntax - STANDARD_MONITOR QA_Tier>WINTESTENV QA_Tier>WINTESTENV WebServicesStats
Rem --------------------------------------------------------------------------------------------------------------------
...
Function printData(objItem)
  ...
  if NOT (objItem.Name = "") then
    vectorName = Replace(objItem.Name, " ","_")
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & objItem.MaximumConnections  & " " & objItem.MeasuredAsyncIOBandwidthUsage & " " & objItem.PostRequestsPersec  & " " & objItem.TotalAllowedAsyncIORequests  & " " & objItem.TotalAnonymousUsers & " " & objItem. TotalBlockedAsyncIORequests  & " " & objItem.Totalblockedbandwidthbytes   & " " & objItem.TotalBytesReceived & " "& objItem.TotalBytesSent & " " & objItem.TotalBytesTransferred & " " & objItem.TotalPostRequests & " " & objItem.ConnectionAttemptsPersec & " " & objItem.CurrentConnections & " " & objItem.ISAPIExtensionRequestsPersec & " " & objItem.OtherRequestMethodsPersec & vbCrlf
