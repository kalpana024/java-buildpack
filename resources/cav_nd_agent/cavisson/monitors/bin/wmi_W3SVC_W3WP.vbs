...
Rem @GDF - cm_win_W3SVC_W3WP.gdf
Rem @Description - This script is used to get stats related to cache, thread, requests.
Rem @Monitor Syntax - STANDARD_MONITOR QA_Tier>WINTESTENV QA_Tier>WINTESTENV W3SVC_W3WPStats
Rem --------------------------------------------------------------------------------------------------------------------
...
Set colItems = objRefresher.AddEnum(objWMIService, _
    "Win32_PerfFormattedData_W3SVCW3WPCounterProvider_W3SVCW3WP").objectSet
...
Function printData(objItem)
  ...
  if NOT (objItem.Name = "") then
    vectorName = Replace(objItem.Name, " ","_")
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & objItem.MaximumThreadsCount  & " " & objItem.OutputCacheCurrentFlushedItems & " " & objItem.OutputCacheCurrentItems  & " " & objItem.OutputCacheCurrentMemoryUsage  & " " & objItem.OutputCacheHitsPersec & " " & objItem. OutputCacheMissesPersec  & " " & objItem.OutputCacheTotalFlushedItems   & " " & objItem.OutputCacheTotalFlushes & " "& objItem.OutputCacheTotalHits & " " & objItem.OutputCacheTotalMisses & " " & objItem.RequestsPerSec & " " & objItem.TotalHTTPRequestsServed & " " & objItem.TotalThreads & vbCrlf
