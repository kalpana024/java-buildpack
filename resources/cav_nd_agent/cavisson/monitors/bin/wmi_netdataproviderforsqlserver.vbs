...
Rem @GDF - cm_win_NETDataProviderForSqlServer_stats.gdf
Rem @Description - This script is used to get metrics related to windows system SqlClient.
Rem @Monitor Syntax - STANDARD_MONITOR QA_Tier>WINTESTENV QA_Tier>WINTESTENV .NETDataProviderForSqlServerStats
Rem --------------------------------------------------------------------------------------------------------------------
...
Set colItems = objRefresher.AddEnum(objWMIService, _
    "Win32_PerfRawData_NETDataProviderforSqlServer_NETDataProviderforSqlServer").objectSet
...
Function printData(objItem)
  ...
  if NOT (objItem.Name = "") then
    vectorName = Replace(objItem.Name, " ","_")
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & objItem.HardConnectsPerSecond & " " & objItem.HardDisconnectsPerSecond & " " & objItem.NumberOfActiveConnectionPoolGroups & " " & objItem.NumberOfActiveConnectionPools & " " & objItem.NumberOfActiveConnections  & " " & objItem.NumberOfFreeConnections & " " & objItem.NumberOfInactiveConnectionPoolGroups & " " & objItem.NumberOfInactiveConnectionPools & " " & objItem.NumberOfNonPooledConnections & " " & objItem.NumberOfPooledConnections  & " " & objItem.NumberOfReclaimedConnections & " " & objItem.NumberOfStasisConnections & " " & objItem.SoftConnectsPerSecond & " " & objItem.SoftDisconnectsPerSecond & vbCrlf
