...
Rem @Program Name - wmi_APP_POOL_WAS.vbs
Rem @GDF - cm_win_APP_POOL_WAS.gdf
Rem @Command - wmic path Win32_PerfFormattedData_APPPOOLCountersProvider_APPPOOLWAS
Rem---------------------------------------------------------------------------------------------------------------------
...
Set colItems = objRefresher.AddEnum(objWMIService, _
    "Win32_PerfFormattedData_APPPOOLCountersProvider_APPPOOLWAS").objectSet
...
Function printData(objItem)
  ...
  if NOT (objItem.Name = "") then
    vectorName = Replace(objItem.Name, " ","_")
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & objItem.CurrentApplicationPoolState  & " " & objItem.CurrentApplicationPoolUptime & " " & objItem.CurrentWorkerProcesses  & " " & objItem.MaximumWorkerProcesses & " " & objItem.RecentWorkerProcessFailures & " " & objItem.TimeSinceLastWorkerProcessFailure & " " & objItem.TotalApplicationPoolRecycles & " " & objItem.TotalApplicationPoolUptime & " " & objItem.TotalWorkerProcessFailures & " " & objItem.TotalWorkerProcessPingFailures & " " & objItem.TotalWorkerProcessShutdownFailures & " " & objItem.TotalWorkerProcessStartupFailures & " " &  objItem.TotalWorkerProcessesCreated &  vbCrlf
  End If
End Function
