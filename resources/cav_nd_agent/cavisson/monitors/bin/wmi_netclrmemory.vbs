...
Function printData(dataobjItem)
  ...
  if NOT (dataobjItem.Name = "") then
    heapUsedMemory = 0
  if Not (dataobjItem.NumberTotalcommittedBytes = 0) then
    heapUsedMemory = (dataobjItem.NumberBytesinallHeaps/dataobjItem.NumberTotalcommittedBytes)*100
  End if
    vectorName = Replace(dataobjItem.Name, " ","_")
    ...
    IF (processIdCmdArgsMap.Exists(processid)) then
      ...
      End if
    End if 
     Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & dataobjItem.AllocatedBytesPersec & " " & dataobjItem.FinalizationSurvivors & " " & Round (dataobjItem.Gen0heapsize/byteToKB,3) & " " & dataobjItem.Gen0PromotedBytesPerSec & " " & Round(dataobjItem.Gen1heapsize/byteToKB,3) & " " & dataobjItem.Gen1PromotedBytesPerSec & " " & Round(dataobjItem.Gen2heapsize/byteToKB,3) & " " & Round(dataobjItem.LargeObjectHeapsize/byteToKB,3) & " " & Round(dataobjItem.NumberBytesinallHeaps/byteToKB,3) & " " & dataobjItem.NumberGCHandles & " " & dataobjItem.NumberGen0Collections & " " & dataobjItem.NumberGen1Collections & " " & dataobjItem.NumberGen2Collections & " " & dataobjItem.NumberInducedGC & " " & dataobjItem.NumberofPinnedObjects & " " & dataobjItem.NumberofSinkBlocksinuse & " " &  Round(dataobjItem.NumberTotalcommittedBytes/byteToKB,3) & " " & Round (dataobjItem.NumberTotalreservedBytes/byteToKB,3) & " " & dataobjItem.PercentTimeinGC & " " & Round(dataobjItem.PromotedFinalizationMemoryfromGen0/byteToKB,3) & " " & Round(dataobjItem.PromotedMemoryfromGen0/byteToKB,3) & " " & Round (dataobjItem.PromotedMemoryfromGen1/byteToKB,3) & " " & heapUsedMemory & vbCrlf
  End If
End Function
