...
Rem @Description - This script provides stats related to database manager object for SQL Server.
Rem @GDF - cm_win_sql_server_database.gdf
Rem @Monitor Syntax - STANDARD_MONITOR QA_Tier>WINTESTENV QA_Tier>WINTESTENV SQLServerDatabase
Rem---------------------------------------------------------------------------------------------
...
Set colItems = objRefresher.AddEnum(objWMIService, _
    "Win32_PerfFormattedData_MSSQLSERVER_SQLServerDatabases").objectSet
...
Function printData(objItem)
  ...
  if NOT (objItem.Name = "") then
    vectorName = Replace(objItem.Name, " ","_")
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" &  objItem.ActiveTransactions & " " & objItem.LogGrowths & " " & objItem.TransactionsPersec & vbCrlf
  End If
End Function
