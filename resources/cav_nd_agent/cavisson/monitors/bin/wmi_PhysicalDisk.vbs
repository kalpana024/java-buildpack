...
Rem @Purpose : The Physical Disk performance object consists of counters that monitor hard or fixed disk drive on a computer. Disks are used to store file, program, and paging data and are read to retrieve these items, and written to record changes to them. The values of physical disk counters are sums of the values of the logical disks (or partitions) into which they are divided.
Rem @GDF    : cm_win_PhysicalDisk.gdf
Rem @Command To Check Counters : wmic path Win32_PerfFormattedData_PerfDisk_PhysicalDisk
Rem @Counter : AvgDiskBytesPerRead  AvgDiskBytesPerTransfer  AvgDiskBytesPerWrite  AvgDiskQueueLength  AvgDiskReadQueueLength  AvgDisksecPerRead  AvgDisksecPerTransfer  AvgDisksecPerWrite  AvgDiskWriteQueueLength  Caption  CurrentDiskQueueLength  Description  DiskBytesPersec  DiskReadBytesPersec  DiskReadsPersec  DiskTransfersPersec  DiskWriteBytesPersec  DiskWritesPersec  Frequency_Object  Frequency_PerfTime  Frequency_Sys100NS  Name    PercentDiskReadTime  PercentDiskTime  PercentDiskWriteTime  PercentIdleTime  SplitIOPerSec  Timestamp_Object  Timestamp_PerfTime  Timestamp_Sys100NS
Rem @Monitor Syntax : STANDARD_MONITOR TierName>Server TierName>ServerName>Instance MonitorName
Rem -----------------------------------------------------------------------------------------------------------------------------------------------------------
...
Set colItems = objRefresher.AddEnum(objWMIService, _
    "Win32_PerfFormattedData_PerfDisk_PhysicalDisk").objectSet
...
Function printData(objItem)
  ...
  vectorName = objItem.Name
  if NOT (vectorName = "") then
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & Round((objItem.AvgDiskBytesPerRead/ByteToMBConvertor),3)  & " " & Round((objItem.AvgDiskBytesPerTransfer/ByteToMBConvertor),3) & " " & Round((objItem.AvgDiskBytesPerWrite/ByteToMBConvertor),3)  & " " & objItem.AvgDiskQueueLength  & " " & objItem.AvgDiskReadQueueLength & " " & objItem. AvgDisksecPerRead  & " " & objItem.AvgDisksecPerTransfer   & " " & objItem.AvgDisksecPerWrite & " "& objItem.AvgDiskWriteQueueLength & " " & objItem.CurrentDiskQueueLength & " " & objItem.DiskBytesPersec & " " & objItem.DiskReadBytesPersec & " " & objItem.DiskReadsPersec & " " & objItem.DiskTransfersPersec & " " & objItem.DiskWriteBytesPersec & " " & objItem.DiskWritesPersec    & " " & objItem.PercentDiskReadTime & " " & objItem.PercentDiskTime& " " & objItem.PercentDiskWriteTime & " " & objItem.PercentIdleTime & " " & objItem.SplitIOPerSec & vbCrlf
  End If
End Function
