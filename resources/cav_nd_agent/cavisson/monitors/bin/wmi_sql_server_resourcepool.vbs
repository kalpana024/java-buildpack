Rem----------------------------------------------------------------------------------------------------------------------
Rem @Author - Divyesh/Nishank
Rem @Description - This class provide the stats related to resource pool.
Rem @GDF -  cm_win_sql_server_resourcepool.gdf
Rem @Mon_Syntx - STANDARD_MONITOR QA_Tier>WINTESTENV QA_Tier>WINTESTENV SQLServerResourcePoolStats
Rem----------------------------------------------------------------------------------------------------------------------
...
Set colItems = objRefresher.AddEnum(objWMIService, _
    "Win32_PerfFormattedData_MSSQLSERVER_SQLServerResourcePoolStats").objectSet
...
Function printData(objItem)
  ...
  if NOT (objItem.Name = "") then
    vectorName = Replace(objItem.Name, " ","_")
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & Round((objItem.ActivememorygrantamountKB/kbToMB),3) & " " & objItem.Activememorygrantscount & " " & objItem.AvgDiskReadIOms & " " & objItem.AvgDiskWriteIOms & " " & Round((objItem.CachememorytargetKB/kbToMB),3) & " " & Round((objItem.CompilememorytargetKB)/kbToMB,3) & " " & objItem.CPUcontroleffectPercent & " " & objItem.CPUusagePercent & " " & objItem.CPUusagetargetPercent & " " & objItem.DiskReadBytesPersec & " " & objItem.DiskReadIOPersec & " " & objItem.DiskReadIOThrottledPersec & " " & objItem.DiskWriteBytesPersec & " " & objItem.DiskWriteIOPersec & " " & objItem.DiskWriteIOThrottledPersec & " " & Round((objItem.MaxmemoryKB/kbToMB),3) & " " & objItem.MemorygrantsPersec & " " & objItem.MemorygranttimeoutsPersec & " " & objItem.Pendingmemorygrantscount & " " & Round((objItem.QueryexecmemorytargetKB/kbToMB),3) & " " & Round((objItem.TargetmemoryKB/kbToMB),3) & " " & Round((objItem.UsedmemoryKB/kbToMB),3) & vbCrlf
  End If
End Function
