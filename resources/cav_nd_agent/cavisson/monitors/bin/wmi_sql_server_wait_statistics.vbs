...
Rem @GDF - cm_win_sql_server_sqlwaitstatistics.gdf
Rem @Description - This script is used to get application thread's wait related information.
Rem @Monitor Syntax - STANDARD_MONITOR QA_Tier>WINTESTENV QA_Tier>WINTESTENV SQLServerSQLWaitStatistics
Rem --------------------------------------------------------------------------------------------------------------------
...
Set colItems = objRefresher.AddEnum(objWMIService, _
    "Win32_PerfFormattedData_MSSQLSERVER_SQLServerWaitStatistics").objectSet
...
Function printData(objItem)
  ...
  if NOT (objItem.Name = "") then
    vectorName = Replace(objItem.Name, " ","_")
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & objItem.Lockwaits & " " & objItem.Logbufferwaits & " " & objItem.Logwritewaits & " " & objItem.Memorygrantqueuewaits & " " & objItem.NetworkIOwaits & " " & objItem.NonPagelatchwaits & " " & objItem.PageIOlatchwaits & " " & objItem.Pagelatchwaits & " " & objItem.Threadsafememoryobjectswaits & " " & objItem.Transactionownershipwaits & " " & objItem.Waitfortheworker & " " & objItem.Workspacesynchronizationwaits & vbCrlf
  End If
End Function
