...
Rem @Component Description :.NET CLR Interop stats. Classes in assemblies that are managed by the common language runtime (CLR) can be accessed in X++ code. This feature is called common language runtime (CLR) interoperability, or CLR interop.
Rem @GDF    : cm_win_logicalDisk_stats.gdf
Rem @Command To Check Counters : wmic path Win32_PerfFormattedData_PerfDisk_LogicalDisk
...
Rem To get All Logical Disk Data : STANDARD_MONITOR QA_Tier>WindowsMachine QA_Tier>WindowsMachine>QA-INST LogicalDiskStats.
Rem To get Specific Logical Disk Data : STANDARD_MONITOR QA_Tier>WindowsMachine QA_Tier>WindowsMachine>QA-INST LogicalDiskStats /args:C,D
Rem ---------------------------------------------------------------------------------------------------------------------
