...
Set dataSetItem = objRefresher.AddEnum(objWMIService, _
    "Win32_PerfFormattedData_NETFramework_NETCLRLoading").objectSet
...
Function printData(dataobjItem)
  ...
  if NOT (dataobjItem.Name = "") then
    ...
    IF (processIdCmdArgsMap.Exists(processid)) then
         ...
         End if
    End if 
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & dataobjItem.TotalClassesLoaded & " " & dataobjItem.CurrentClassesLoaded & " " & dataobjItem.RateofClassesLoaded & " " & dataobjItem.TotalNumberofLoadFailures & " " & dataobjItem.RateofLoadFailures & " " & Round((dataobjItem.BytesinLoaderHeap)/byteToMB,3) & " " & dataobjItem.PercentTimeLoading & " " & dataobjItem.TotalAppdomains & " " & dataobjItem.Currentappdomains & " " & dataobjItem.Rateofappdomains & " " & dataobjItem.Totalappdomainsunloaded & " " & dataobjItem.Rateofappdomainsunloaded & " " & dataobjItem.TotalAssemblies & " " & dataobjItem.CurrentAssemblies & " " & dataobjItem.RateofAssemblies & " " & dataobjItem.AssemblySearchLength & vbCrlf
  End If
End Function
