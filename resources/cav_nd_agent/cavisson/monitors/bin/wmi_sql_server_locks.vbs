...
Rem @GDF - cm_win_sql_server_locks.gdf
Rem @Description - This script used to get statistics for individual lock server lock requests 
Rem Monitor Syntax - STANDARD_MONITOR QA_Tier>WINTESTENV QA_Tier>WINTESTENV SQLServerLocks
Rem-----------------------------------------------------------------------------------------
...
Set colItems = objRefresher.AddEnum(objWMIService, _
    "Win32_PerfFormattedData_MSSQLSERVER_SQLServerLocks").objectSet
...
Function printData(objItem)
  ...
  if NOT (objItem.Name = "") then
    vectorName = Replace(objItem.Name, " ","_")
    Wscript.Echo getVectorID(vectorName) & ":" & vectorName & "|" & objItem.AverageWaitTimems & " " & objItem.LockRequestsPersec & " " & objItem.LockTimeoutsPersec & " " & objItem.LockTimeoutstimeout0Persec & " " & objItem.LockWaitsPersec & " " & objItem.LockWaitTimems & " " & objItem.NumberofDeadlocksPersec & vbCrlf
  End If
End Function
